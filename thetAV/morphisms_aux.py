...
def constant_f(g, A, C):
    ...
    prod_list = Counter(
        list(product(A & U, U - A)) + list(product((B - A) & (B - U), A - U))
    )
    div_list = Counter(
        list(product((A ^ C) & (U ^ C), (U ^ C) - (A ^ C)))
        + list(product((B - (A ^ C)) & (B - (U ^ C)), (A ^ C) - (U ^ C)))
    )
    ff *= prod(
        (compatible_sqrt(g, k, i) ** m for (i, k), m in (prod_list - div_list).items()),
        EpElement(),
    )
    ff /= prod(
        (compatible_sqrt(g, k, i) ** m for (i, k), m in (div_list - prod_list).items()),
        EpElement(),
    )
...
def choice_of_C_Cosset(g, A):
    ...
    Cp = (
        set()
    )  # 2*g + 1 in Cp  if 2*g + 1  not in A DIFF shouldn't it be Cp = {2*g + 1} then?
    Cpp = set()
...
def constant_f2_level2(a, thc, A, C):
    ...
    # the two theta constants which appear in the definition
    f2 = (
        thO[eta(g, C, normalized=True, idx=True)]
        / thO[eta(g, (U ^ A) ^ C, normalized=True, idx=True)]
    )

    # the four product of the definitions
    f2 *= prod((-1) ** (k < i) * (a[k] - a[i]) for i, k in product(A & U, U - A))
    f2 *= prod(
        (-1) ** (k < i) * (a[k] - a[i]) for i, k in product((B - A) & (B - U), A - U)
    )
    f2 /= prod(
        (-1) ** (k < i) * (a[k] - a[i])
        for i, k in product((A ^ C) & (U ^ C), (U ^ C) - (A ^ C))
    )
    f2 /= prod(
        (-1) ** (k < i) * (a[k] - a[i])
        for i, k in product((B - (A ^ C)) & (B - (U ^ C)), (A ^ C) - (U ^ C))
    )
...
def YS_fromMumford_Generic(g, a, S, points):
    ...
    if len(points) != g or len(set(points)) != g:
        raise ValueError(
            f"Expected points={points} to be a list of {g} distinct elements"
        )
    ...
    for I in combinations(range(g), n):
        ...
        t *= prod(
            points[k][0] - a[l] for l, k in product(S, range(g)) if k not in I
        )  # (x_k-a_l)
        t /= prod(
            points[i][0] - points[k][0] for i, k in product(I, range(g)) if k not in I
        )
        YS += t
    return YS
...
def YS_fromMumford_Delta(g, a, S, points, F):  # DIFF: Not tested against Magma
    ...
    if len(points) != g or len(set(points)) != g - 1:
        raise ValueError(
            f"Expected points={points} to be a list of {g} elements of which exactly two are equal"
        )
    ...
    for I in chain.from_iterable(
        combinations(range(g - 2), r) for r in [n, n - 2] if r >= 0
    ):
        if len(I) == n - 2:
            I = I + (
                g - 2,
                g - 1,
            )  # DIFF: In Magma implementation, in this case, missing y_{-1}^2 ? See Definition 5.1.26 on pg 116
        t = prod(points[i][1] for i in I)
    ...
    P = prod(x - a[l] for l in range(2 * g + 1) if l not in S) * prod(
        y - a[l] for l in S
    )
    P -= prod(y - a[l] for l in range(2 * g + 1) if l not in S) * prod(
        x - a[l] for l in S
    )
    try:
        P = K(P / (x - y))
    ...
    t01 = (
        P(points[-1][0], points[-1][0])
        / (2 * points[-1][1])
        * prod(points[-1][0] - a[l] for l in S)
    )
    ...
    for I in combinations(range(g - 2), n - 1):
        ...
        t1 /= prod(
            ...
            for i, k in product(I + (-1,), range(g - 2))
            if k not in I
        )
        ...
        t2 *= prod(
            points[k][0] - a[l] for l, k in product(S, range(g - 2)) if k not in I
        )
        t2 /= prod(
            points[i][0] - points[k][0]
            for i, k in product(I, range(g - 2))
            if k not in I
        )

        P = prod(points[i][0] - x for i in I) * prod(
            y - points[k][0] for k in range(g - 2) if k not in I
        )
        P -= prod(points[i][0] - y for i in I) * prod(
            x - points[k][0] for k in range(g - 2) if k not in I
        )
        try:
            P = K(P / (x - y))
        except TypeError:
...
def prodYp_fromMumford_with2torsion(g, a, S, points, V, C, F):
    ...
    Y = F(0)
    for Ip in product(
        *[combinations(rangeS(g, R), ni - len(V & si)) for ni, si in zip(n, S)]
    ):
...
def Y_fromMumford_with2torsion(g, a, S, points, V):
    ...
    for I, J in product(combinations(range(v, g), n - v), repeat=2):
        ...
        for elem in [I, J]:
            t *= prod(
                points[k][0] - a[l]
                for l, k in product(S - V, range(v, g))
                if k not in elem
            )
            t /= prod(
                points[i][0] - points[k][0]
                for i, k in product(elem, range(v, g))
                if k not in elem
            )
        Y += t
